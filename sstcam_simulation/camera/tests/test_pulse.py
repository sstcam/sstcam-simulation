...
def test_reference_pulses(ref_pulse_class):
    ...
    np.testing.assert_allclose(reference_pulse.pulse.sum() * CONTINUOUS_READOUT_SAMPLE_WIDTH, 1)
...
def test_generic_pulse():
    ...
    np.testing.assert_allclose(reference_pulse.pulse.sum() * CONTINUOUS_READOUT_SAMPLE_WIDTH, 1)
    assert reference_pulse.time.size == reference_pulse.pulse.size
    assert reference_pulse.pulse.size == 60 / CONTINUOUS_READOUT_SAMPLE_WIDTH
