...
def FlasherChargeResolution():
    ...
    for n_events in [100,1000]:
        ...
        plt.plot(x, poisson_limit, label="Poisson Limit " + str(n_events) + " events")
        plt.plot(x, enf_limit, label="ENF Limit "  + str(n_events) + " events", color='black')
        plt.plot(x, y, '.', label="From Waveform" + str(n_events) + " events")
...
# FlasherChargeResolution()

def FlasherChargeResolutionNSB():
    ...
    for nsb_MHz in [50, 100, 200, 300]:
        ...
        plt.plot(x, y, '.', label="From Waveform NSB " + str(nsb_MHz) + " MHz")
...
# FlasherChargeResolutionNSB()

import random
def FlasherChargeResolutionFluctuatingNSB():
    ...
    n_events=1000
    #for n_events in range(0,100,50):
    nsb_mhz=[50,100,150,200,250,300]
    ...
    for illumination in tqdm(np.logspace(1, 3, 10)): # up to 1000 illumination in log space
        for iev in range(n_events):
            flasher = source.get_flasher_illumination(time=50, illumination=illumination,
                                                      ...
                                                      pulse_width_err=0.035)
            # Add in NSB
            nsb = source.get_nsb(rate=random.choice(nsb_mhz) )
            pe = flasher + nsb
    ...
    plt.plot(x, y, '.', label="From Waveform 50 ns start " )
...
def ChargeResolutionNSB():
    ...
    for nsb_MHz in [50, 100, 200, 300]:
        for illumination in tqdm(np.logspace(1, 3, 10)): # up to 1000 illumination in log space
            for iev in range(n_events):
                #flasher = source.get_flasher_illumination(time=1, illumination=illumination,
                #                                          flasher_pulse_width=4.5,
                #                                          illumination_err=0.2,
                #                                          pulse_width_err=0.035)
                # Add in NSB
    ...
    for nsb_MHz in [50, 100, 200, 300, 1000]: # select and plot appropriate NSB
        ...
        for nsb, illumination, truecharge, measuredcharge in zip(nsbs, illuminations, truecharges, measuredcharges):
            if nsb==nsb_MHz:
                ...
                #plot_charge_errs.append(100-(100*truecharge/measuredcharge))
...
# 100 and 200 p.e. - no AC coupling - no pedestal subtraction - NSB constant from a range of values
# 12/6/21
# How many flashes to get fractional error to minimum
...
# 26 June 2021 - Now make the above more configurable so we can seperate iteration conditions better
...
# 26/6/2021
flasher_illuminations=[100,200]
nsb_rates=[50, 100, 200, 300, 1000]
pulse_width=4.5
illumination_err=0.2
pulse_width_err=0.035
no_of_nsb_obs=20
